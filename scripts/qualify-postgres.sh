...
migrations=(
  ...
  "$root/infrastructure/postgres/013-team-invitations.sql"
  "$root/infrastructure/postgres/014-action-workflows.sql"
  "$root/infrastructure/postgres/015-reasoning-graph.sql"
  "$root/infrastructure/postgres/016-execution-lineage-indexes.sql"
  "$root/infrastructure/postgres/017-causal-traversal-indexes.sql"
  "$root/infrastructure/postgres/018-protected-branch-governance.sql"
)
...
insert into principals(id,kind,display_name,email) values('principal_reviewer','human','Qualification Reviewer','reviewer@example.invalid') on conflict(id) do nothing;
...
insert into workspace_memberships(workspace_id,principal_id,role) values('workspace_qualification','principal_reviewer','admin') on conflict do nothing;
...
insert into sessions_repository_refs(repository_id,ref_type,name,checkpoint_id,metadata) values('repo_qualification','branch','feature','cp_native_qualification','{"id":"ws_native_feature"}') on conflict(repository_id,ref_type,name) do update set checkpoint_id=excluded.checkpoint_id,metadata=excluded.metadata,updated_at=now();
...
insert into repository_branch_policies(workspace_id,repository_id,branch_name,required_approvals,required_human_approvals,require_independent_approval,require_verification,require_actions_success,block_changes_requested,created_by)
values('workspace_qualification','repo_qualification','main',1,1,true,true,true,true,'principal_qualification')
on conflict(repository_id,branch_name) do update set required_approvals=1,required_human_approvals=1,require_independent_approval=true,require_verification=true,require_actions_success=true,block_changes_requested=true,updated_at=now();

insert into pull_requests(workspace_id,repository_id,number,title,base_branch,head_branch,head_commit_id,author_principal_id,verification_state,required_approvals)
values('workspace_qualification','repo_qualification',2,'Protected merge qualification','main','feature','cp_native_qualification','principal_qualification','passed',1)
on conflict(repository_id,number) do update set state='open',verification_state='passed',head_commit_id='cp_native_qualification';
insert into pull_request_reviews(pull_request_id,reviewer_principal_id,state,body)
select id,'principal_reviewer','approved','independent human approval' from pull_requests where repository_id='repo_qualification' and number=2
on conflict do nothing;
insert into action_runs(id,workspace_id,repository_id,commit_id,pull_request_id,trigger,status,conclusion,actor_principal_id,started_at,completed_at)
select '00000000-0000-0000-0000-000000000201','workspace_qualification','repo_qualification','cp_native_qualification',id,'pull_request','completed','success','principal_reviewer',now(),now()
from pull_requests where repository_id='repo_qualification' and number=2
on conflict(id) do update set status='completed',conclusion='success',completed_at=now();
insert into action_checks(id,action_run_id,name,category,status,conclusion,summary,started_at,completed_at)
values('00000000-0000-0000-0000-000000000202','00000000-0000-0000-0000-000000000201','Protected branch qualification','policy','completed','success','independent review, verification and action evidence satisfied',now(),now())
on conflict(action_run_id,name) do update set status='completed',conclusion='success',completed_at=now();
update pull_requests set state='merged',merged_at=now() where repository_id='repo_qualification' and number=2;

insert into pull_requests(workspace_id,repository_id,number,title,base_branch,head_branch,head_commit_id,author_principal_id,verification_state,required_approvals)
values('workspace_qualification','repo_qualification',3,'Self approval rejection qualification','main','feature','cp_native_qualification','principal_qualification','passed',1)
on conflict(repository_id,number) do update set state='open',verification_state='passed',head_commit_id='cp_native_qualification';
insert into pull_request_reviews(pull_request_id,reviewer_principal_id,state,body)
select id,'principal_qualification','approved','self approval must not count' from pull_requests where repository_id='repo_qualification' and number=3
on conflict do nothing;
DO $$
BEGIN
  BEGIN
    update pull_requests set state='merged',merged_at=now() where repository_id='repo_qualification' and number=3;
    raise exception 'protected branch self-approval rejection assertion failed';
  EXCEPTION WHEN OTHERS THEN
    if sqlerrm = 'protected branch self-approval rejection assertion failed' then raise; end if;
  END;
END $$;
...
BEGIN
  ...
  IF (select count(*) from workspace_memberships where workspace_id='workspace_qualification') <> 2 THEN RAISE EXCEPTION 'tenancy assertion failed'; END IF;
  ...
  IF (select count(*) from pull_requests where repository_id='repo_qualification') < 3 THEN RAISE EXCEPTION 'pull request persistence assertion failed'; END IF;
  IF (select state from pull_requests where repository_id='repo_qualification' and number=2) <> 'merged' THEN RAISE EXCEPTION 'protected branch valid merge assertion failed'; END IF;
  IF (select state from pull_requests where repository_id='repo_qualification' and number=3) <> 'open' THEN RAISE EXCEPTION 'protected branch self approval was not rejected'; END IF;
  IF (select count(*) from repository_branch_policies where repository_id='repo_qualification' and branch_name='main') <> 1 THEN RAISE EXCEPTION 'protected branch policy persistence assertion failed'; END IF;
  ...
  IF to_regclass('public.repository_action_workflows') is null OR to_regclass('public.engineering_memory') is null THEN RAISE EXCEPTION 'current migration coverage assertion failed'; END IF;
END $$;
SQL
...
BEGIN
  ...
  IF (select count(*) from pull_requests where repository_id='repo_qualification') < 3 THEN RAISE EXCEPTION 'restored PR assertion failed'; END IF;
  ...
  IF (select count(*) from repository_branch_policies where repository_id='repo_qualification' and branch_name='main') <> 1 THEN RAISE EXCEPTION 'restored protected branch policy assertion failed'; END IF;
  ...
  IF to_regclass('public.repository_action_workflows') is null OR to_regclass('public.engineering_memory') is null THEN RAISE EXCEPTION 'restored current schema assertion failed'; END IF;
END $$;
SQL
printf 'PostgreSQL qualification passed: all current migrations, Sessions-native source control, protected branch governance, collaboration, persistence, backup, and independent restore verified.\n'
