...
"${compose[@]}" build --pull api auth billing web runner
...
"${compose[@]}" up -d --no-deps api auth billing runner
"${compose[@]}" up -d --no-deps web
"${compose[@]}" up -d --no-deps proxy

for i in $(seq 1 60); do
  api_ok=0; billing_ok=0; auth_ok=0
  ...
  if [[ "$api_ok" == 1 && "$billing_ok" == 1 && "$auth_ok" == 1 ]]; then
