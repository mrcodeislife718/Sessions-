...
if (databaseUrl) execFileSync("psql", [databaseUrl, "-v", "ON_ERROR_STOP=1", "-f", "infrastructure/postgres/009-lifecycle-evidence-events.sql"], { stdio: "inherit" });
...
for (const [type, payload] of [["HumanActionRecorded", { intent: "implement qualification target" }],["FileChanged", { path: "src/qualification.ts", change: "created" }],["CommandExecuted", { command: "npm test", exitCode: 0 }]]) await request(`/api/sessions/${sessionId}/events`, { method: "POST", body: JSON.stringify({ type, payload }) });
const snapshot = await request(`/api/sessions/${sessionId}/snapshots`, { method: "POST", body: JSON.stringify({ entries: [{ path: "src/qualification.ts", content: "export const qualified = true;\n" }] }) });
assert.ok(snapshot.id);
const verification = await request(`/api/sessions/${sessionId}/verifications`, { method: "POST", body: JSON.stringify({ snapshotId: snapshot.id, kind: "test", status: "passed", summary: "qualification tests passed" }) });
...
for (const expected of ["SessionStarted", "HumanActionRecorded", "FileChanged", "CommandExecuted", "SnapshotCreated", "VerificationPassed", "DeploymentStarted", "DeploymentCompleted"]) assert.ok(eventTypes.has(expected), `missing recovered lifecycle event ${expected}`);
