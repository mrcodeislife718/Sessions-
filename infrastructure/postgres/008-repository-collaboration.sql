CREATE TABLE IF NOT EXISTS repository_issues (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(), workspace_id uuid NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE, repository_id text NOT NULL,
  ...
  author_principal_id uuid REFERENCES principals(id), assignee_principal_id uuid REFERENCES principals(id), labels jsonb NOT NULL DEFAULT '[]'::jsonb,
...
CREATE TABLE IF NOT EXISTS issue_comments (
  ...
  author_principal_id uuid REFERENCES principals(id), body text NOT NULL, provenance jsonb NOT NULL DEFAULT '{}'::jsonb,
...
CREATE TABLE IF NOT EXISTS pull_requests (
  ...
  base_branch text NOT NULL, head_branch text NOT NULL, head_commit_id text, merge_commit_id text, author_principal_id uuid REFERENCES principals(id),
...
CREATE TABLE IF NOT EXISTS pull_request_comments (
  ...
  author_principal_id uuid REFERENCES principals(id), body text NOT NULL, path text, line integer, provenance jsonb NOT NULL DEFAULT '{}'::jsonb,
  created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now()
);

CREATE TABLE IF NOT EXISTS pull_request_reviews (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(), pull_request_id uuid NOT NULL REFERENCES pull_requests(id) ON DELETE CASCADE,
  reviewer_principal_id uuid REFERENCES principals(id), state text NOT NULL CHECK (state IN ('commented','approved','changes_requested','dismissed')),
...
CREATE TABLE IF NOT EXISTS action_runs (
  ...
  actor_principal_id uuid REFERENCES principals(id), started_at timestamptz, completed_at timestamptz, created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS repository_releases (
  ...
  author_principal_id uuid REFERENCES principals(id), created_at timestamptz NOT NULL DEFAULT now(), published_at timestamptz,
...
CREATE TABLE IF NOT EXISTS repository_deployments (
  ...
  evidence jsonb NOT NULL DEFAULT '{}'::jsonb, actor_principal_id uuid REFERENCES principals(id), created_at timestamptz NOT NULL DEFAULT now(), completed_at timestamptz
);
