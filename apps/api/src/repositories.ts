...
function pushedHead(input:any):string|null{
  const activeWorkstreamId=input?.state?.state?.activeWorkstreamId;
  const refs=Array.isArray(input?.refs)?input.refs:[];
  const active=refs.find((ref:any)=>(ref.refType??ref.ref_type)==="branch"&&String(ref?.metadata?.id??"")===String(activeWorkstreamId??""));
  const fallback=refs.find((ref:any)=>(ref.refType??ref.ref_type)==="branch"&&(ref.checkpointId??ref.checkpoint_id));
  return String(active?.checkpointId??active?.checkpoint_id??fallback?.checkpointId??fallback?.checkpoint_id??"")||null;
}

async function enqueuePushAction(client:PoolClient,identity:Identity,repositoryId:string,commitId:string|null){
  if(!commitId)return null;
  const checkpoint=await client.query("select 1 from sessions_repository_checkpoints where repository_id=$1 and checkpoint_id=$2",[repositoryId,commitId]);
  if(!checkpoint.rowCount)throw new RepositoryTransportError(400,"active branch head does not reference a stored Sessions commit");
  const existing=await client.query("select id,status,conclusion from action_runs where workspace_id=$1 and repository_id=$2 and commit_id=$3 and trigger='sessions.push' order by created_at desc limit 1",[identity.workspaceId,repositoryId,commitId]);
  if(existing.rowCount)return existing.rows[0];
  const run=await client.query("insert into action_runs(workspace_id,repository_id,commit_id,trigger,status,actor_principal_id) values($1,$2,$3,'sessions.push','queued',$4) returning *",[identity.workspaceId,repositoryId,commitId,identity.principalId]);
