...
function enforceRateLimit(req: http.IncomingMessage) {
  const key = clientKey(req), now = Date.now(), prior = attempts.get(key);
  const bucket = !prior || prior.resetAt <= now ? { count: 0, resetAt: now + 60_000 } : prior;
  bucket.count += 1; attempts.set(key, bucket);
  if (bucket.count > rateLimit) throw new HttpError(429, "too many authentication attempts; try again shortly");
}
...
function validatePlan(plan: unknown) { const value = String(plan ?? "developer"); if (!/^(developer|team|business|enterprise)$/.test(value)) throw new HttpError(400, "invalid paid plan"); return value; }
...
async function verifyPassword(password: string, salt: string, expected: string) {
  const actual = Buffer.from(await passwordHash(password, salt), "hex"); const target = Buffer.from(expected, "hex");
...
async function createCredential(db: PoolClient, workspaceId: string, principalId: string) {
  const token = issueToken();
  await db.query("insert into api_credentials(id,workspace_id,principal_id,token_hash,scopes) values($1,$2,$3,$4,$5)", [`credential_${randomUUID()}`, workspaceId, principalId, hashToken(token), ownerScopes]);
  return token;
}

async function signup(body: any) {
  const email = String(body.email ?? "").trim(), normalized = normalizeEmail(email), password = String(body.password ?? ""), displayName = String(body.displayName ?? "").trim(), workspaceName = String(body.workspaceName ?? "").trim(), plan = validatePlan(body.planKey);
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalized)) throw new HttpError(400, "valid email is required");
  if (displayName.length < 2 || displayName.length > 120) throw new HttpError(400, "display name is required");
  if (workspaceName.length < 2 || workspaceName.length > 120) throw new HttpError(400, "workspace name is required");
  validatePassword(password);
  const salt = randomBytes(16).toString("hex"), digest = await passwordHash(password, salt), id = ids();
  const client = await pool.connect();
  try {
    await client.query("begin");
    const duplicate = await client.query("select 1 from hosted_auth_accounts where normalized_email=$1", [normalized]);
    if (duplicate.rowCount) throw new HttpError(409, "an account already exists for this email");
    ...
    await client.query("insert into product_events(id,workspace_id,principal_id,event_name,properties) values($1,$2,$3,'self_service_signup',$4)", [`product_${randomUUID()}`, id.workspace, id.principal, JSON.stringify({ plan })]);
    const token = await createCredential(client, id.workspace, id.principal);
    await client.query("commit");
    return { token, workspaceId: id.workspace, principalId: id.principal, planKey: plan, entitlementStatus: "suspended" };
  } catch (error: any) { await client.query("rollback"); if (error?.code === "23505") throw new HttpError(409, "an account already exists for this email"); throw error; }
...
async function login(body: any) {
  ...
  if (account.principal_status !== "active") throw new HttpError(403, "account is not active");
  if (account.locked_until && new Date(account.locked_until).getTime() > Date.now()) throw new HttpError(429, "account temporarily locked");
  if (!(await verifyPassword(password, account.password_salt, account.password_hash))) {
    ...
    await pool.query("update hosted_auth_accounts set failed_attempts=$2,locked_until=case when $2>=8 then now()+interval '15 minutes' else null end,updated_at=now() where principal_id=$1", [account.principal_id, failures]);
    throw new HttpError(401, "invalid email or password");
  }
  const client = await pool.connect();
  try { await client.query("begin"); const token = await createCredential(client, account.workspace_id, account.principal_id); await client.query("update hosted_auth_accounts set failed_attempts=0,locked_until=null,last_login_at=now(),updated_at=now() where principal_id=$1", [account.principal_id]); await client.query("commit"); return { token, workspaceId: account.workspace_id, principalId: account.principal_id }; }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/health") { await pool.query("select 1"); return send(res, 200, { ok: true, service: "sessions-auth" }); }
    if (req.method === "POST" && url.pathname === "/api/auth/signup") { enforceRateLimit(req); return send(res, 201, await signup(await jsonBody(req))); }
    if (req.method === "POST" && url.pathname === "/api/auth/login") { enforceRateLimit(req); return send(res, 200, await login(await jsonBody(req))); }
...
server.listen(port, "0.0.0.0", () => console.log(JSON.stringify({ level: "info", event: "auth.started", port })));
