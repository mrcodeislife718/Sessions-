  ...
  return (
    <main className="session-page">
      <header className="session-topbar">
        ...
        <div className="session-top-actions"><span className="capture-live"><i></i> Connected</span><Link href="/dashboard" className="button button-secondary">Repositories</Link></div>
      </header>
      ...
      <section className="session-header section-narrow">
        ...
        <div className="session-summary-strip">
          ...
          <div><span>Activity</span><strong>{events.length}</strong></div>
          <div><span>Commits</span><strong>{snapshots.length}</strong></div>
          <div><span>Checks</span><strong>{verifications.length - failed.length}/{verifications.length} passed</strong></div>
          <div><span>Recovery</span><strong className={latestSnapshot ? "success-text" : "warning-text"}>{latestSnapshot ? "Commit available" : "No commit"}</strong></div>
        </div>
      </section>
      ...
      <section className="session-grid section-narrow">
        <div className="session-column-main">
          <article className="workspace-panel detail-panel">
            <div className="panel-head"><div><h2>Activity</h2><p>Repository activity with human and AI attribution.</p></div></div>
            <div className="timeline">
              {events.length === 0 ? <p>No activity captured yet.</p> : events.map((event, index) => {
          ...
          <article className="workspace-panel detail-panel">
            <div className="panel-head"><div><h2>Commits</h2><p>Immutable source states with Sessions verification, provenance, and recovery data.</p></div></div>
            <div className="file-list">
              {snapshots.length === 0 ? <p>No commits recorded yet. Use <code>sessions commit</code>.</p> : snapshots.map((snapshot) => (
        ...
        <aside className="session-column-side">
          <article className="workspace-panel detail-panel sticky-panel">
            <div className="panel-head"><div><h2>Checks</h2><p>Verification evidence attached to this Session.</p></div></div>
            <div className="check-list">
              {verifications.length === 0 ? <p>No checks yet.</p> : verifications.map((check) => (
            ...
            {failed.length > 0 && <div className="gate-callout"><strong>Review required</strong><p>{failed.length} check(s) are not passing.</p></div>}
          </article>

          <article className="workspace-panel detail-panel">
            <div className="panel-head"><div><h2>Recovery</h2><p>Restore from the nearest recorded commit.</p></div></div>
            {latestSnapshot ? <div className="checkpoint-card"><span>Latest commit</span><strong>{latestSnapshot.id}</strong><p>Digest {latestSnapshot.digest.slice(0, 16)}…</p><code>sessions restore {latestSnapshot.id}</code></div> : <p>Create a commit before consequential changes.</p>}
          </article>
        </aside>
      </section>
    </main>
