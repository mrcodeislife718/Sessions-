    ...
    <section className="sessions-main">
      ...
      <div className="sessions-content">
        ...
        {state?<>
          ...
          <section className="sessions-overview-grid"><section className="sessions-section" id="issues"><div className="sessions-section-title"><h2>Issues</h2><span>{openIssues} open</span></div><div className="session-feed">{issues.length?issues.map(issue=><div className="feed-row" key={issue.id}><span className="feed-symbol">○</span><span className="feed-copy"><strong>#{issue.number} {issue.title}</strong><span>{issue.state}</span></span><time>{new Date(issue.created_at).toLocaleDateString()}</time></div>):<div className="sessions-empty">No Issues yet.</div>}</div></section><section className="sessions-section" id="pull-requests"><div className="sessions-section-title"><div><h2>Pull requests</h2><span>{openPulls} open</span></div><Link className="button button-primary" href={`/repositories/${encodeURIComponent(id)}/pulls/new`}>New pull request</Link></div><div className="session-feed">{pulls.length?pulls.map(pr=><Link className="feed-row" key={pr.id} href={`/repositories/${encodeURIComponent(id)}/pulls/${pr.number}`}><span className="feed-symbol">⑂</span><span className="feed-copy"><strong>#{pr.number} {pr.title}</strong><span>{pr.head_branch} → {pr.base_branch} · {pr.verification_state}{pr.draft?" · draft":""}</span></span><time>{pr.state}</time></Link>):<div className="sessions-empty"><strong>No Pull requests yet.</strong>Open one to verify and review native Sessions changes before merge.</div>}</div></section></section>
          <section className="sessions-section" id="actions"><div className="sessions-section-title"><h2>Actions</h2><span>Automatic evidence on native push</span></div><div className="session-feed">{actions.length?actions.map(run=><div key={run.id}><div className="feed-row"><span className="feed-symbol">▶</span><span className="feed-copy"><strong>{run.trigger}</strong><span>{run.commit_id??"repository"} · {run.status} · {run.checks?.length??0} checks</span></span><time>{run.conclusion??"pending"}</time></div>{run.checks?.map(check=><div className="feed-row" key={check.id}><span className="feed-symbol">{check.conclusion==="success"?"✓":check.conclusion==="failure"?"×":"·"}</span><span className="feed-copy"><strong>{check.name}</strong><span>{check.summary||`${check.category} · ${check.status}`}</span></span><time>{check.conclusion??check.status}</time></div>)}</div>):<div className="sessions-empty">No Actions runs yet. A native <code>sessions push</code> queues verification automatically.</div>}</div></section>
